    ...
    <div className="flex gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <svg
          key={i}
          className={`h-4 w-4 ${
            i < rating ? "text-yellow-400" : "text-surface-700"
          }`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
...
export default function TestimonialsSection() {
  return (
    <section className="relative bg-surface-900 py-24 lg:py-32">
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute right-1/4 top-0 h-[400px] w-[400px] rounded-full bg-primary-600/5 blur-[100px]" />
      </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:gap-8">
          {testimonialsData.map((t, i) => (
            ...
            >
              {/* Quote */}
              <svg
                className="mb-4 h-8 w-8 text-primary-500/30"
                fill="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <p className="mb-6 text-sm leading-relaxed text-surface-200/70 lg:text-base">
